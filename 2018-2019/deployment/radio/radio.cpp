...
RFM69::RFM69(uint8_t slaveSelectPin, uint8_t interruptPin, bool isRFM69HW, SPIClass * spiDevice)
{
...
void RFM69::interruptHandler() {
  ...
  if (_mode == RF69_MODE_RX && (readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY))
  {
    ...
    setMode(RF69_MODE_STANDBY);
    select();
    mySPI->transfer(REG_FIFO & 0x7F);
    PAYLOADLEN = mySPI->transfer(0);
    PAYLOADLEN = PAYLOADLEN > 66 ? 66 : PAYLOADLEN; // precaution
    TARGETID = mySPI->transfer(0);
    ...
    SENDERID = mySPI->transfer(0);
    uint8_t CTLbyte = mySPI->transfer(0);
...
#endif
    select();
    mySPI->transfer(REG_AESKEY1 | 0x80);
    for (uint8_t i = 0; i < 16; i++)
      mySPI->transfer(key[i]);
    unselect();
  }
...
{
  select();
  mySPI->transfer(addr & 0x7F);
  uint8_t regval = mySPI->transfer(0);
  unselect();
  return regval;
}
...
#ifdef SPI_HAS_TRANSACTION
  mySPI->beginTransaction(_settings);
#endif
...
#else
  mySPI->setClockDivider(SPI_CLOCK_DIV4); // decided to slow down from DIV2 after SPI stalling in some instances, especially visible on mega1284p when RFM69 and FLASH chip both present
#endif
...
void RFM69::readAllRegs()
{
  ...
  {
    select();
    mySPI->transfer(regAddr & 0x7F); // send address + r/w bit
    regVal = mySPI->transfer(0);
    unselect();
...
void RFM69::listenModeInterruptHandler(void)
{
  ...
  SENDERID = mySPI->transfer(0);
  burstRemaining.b[0] =  mySPI->transfer(0);  // and get the time remaining
  burstRemaining.b[1] =  mySPI->transfer(0);
...
void RFM69::listenModeSendBurst( uint8_t targetNode, void* buffer, uint8_t size )
{
  ...
  while(timeRemaining.l > 0) {
    ...
    select();
    mySPI->transfer(REG_FIFO | 0x80);
    mySPI->transfer(size + 4);      // two bytes for target and sender node, two bytes for the burst time remaining
    mySPI->transfer(targetNode);
    mySPI->transfer(_address);

    // We send the burst time remaining with the packet so the receiver knows how long to wait before trying to reply
    mySPI->transfer(timeRemaining.b[0]);
    mySPI->transfer(timeRemaining.b[1]);
